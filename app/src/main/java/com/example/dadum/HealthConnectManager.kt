...
class HealthConnectManager(private val context: Context){
    ...
    suspend fun checkPermissionsAndRun(requestPermission: ActivityResultLauncher<Set<String>>): Boolean {
        val granted = healthConnectClient.permissionController.getGrantedPermissions()
        return if (granted.containsAll(permissions)) {
            Log.d("HEALTH_SYNC","모든 헬스커넥트 권한이 이미 허용됨")
            true
        } else {
            Log.d("HEALTH_SYNC","헬스커넥트 권한 요청 실행")
    ...
    suspend fun readSleepSessions(): List<SleepSessionRecord>{
        val now = Instant.now()
        val todayStart = LocalDate.now().atStartOfDay(ZoneId.systemDefault()).toInstant()//오늘 자정시간 기준
